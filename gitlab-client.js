...
module.exports = class Gitlab {
  ...
  projectTags(projectId) {
    return this.client.url(`/projects/${encodeURIComponent(projectId)}/repository/tags`).get().json()
  }

  // https://docs.gitlab.com/ee/api/jobs.html#download-the-artifacts-archive
  downloadArtifacts(projectId, refName, job) {
    return this.client.url(`/projects/${encodeURIComponent(projectId)}/jobs/artifacts/${refName}/download?job=${job}`).get().res(res => {
      console.log('res', res)
      return res.body
    })
  }
